...
def animate():
    ...
    background_image = pygame.image.load("bg_day.jpg")
    screen.blit(background_image, (-80, -50))
...
while True:
    ...
    if hit:
        # 撞到树时候发生的事情
        if hit[0].type == "a" and not hit[0].passed:   
            points -= 0.31
            ...
            hit[0].passed = True
        # 碰到旗子时候
        elif hit[0].type == "b" and not hit[0].passed:   
            points += 0.3
            obstacles.remove(hit[0])                    
    
    score_text = font.render("GPA: " + str(points), 1, (0, 0, 0))
    animate()
